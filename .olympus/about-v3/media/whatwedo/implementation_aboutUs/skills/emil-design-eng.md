<!-- ============================================================================
     SCOPE NOTE — read before applying anything below.

     This skill is written for UI COMPONENTS (buttons, toasts, dropdowns,
     popovers, drawers). This project's Phase D.2 is a SCROLL-SCRUBBED CINEMATIC
     SEQUENCE. Roughly half of this file does not transfer. See
     ../IMPLEMENTATION-PLAN.md §6.2 for the exact in-scope / out-of-scope split.

     IN SCOPE  — blur to mask an imperfect crossfade; never animate from scale(0);
                 transform+opacity only; never `transition: all`; never ease-in on
                 anything entering; clip-path inset reveals; stagger 30-80ms;
                 asymmetric enter/exit; CSS beats JS under main-thread load;
                 reduced-motion means gentler not zero; slow-motion debugging;
                 the Before/After review table.

     OUT OF SCOPE — the duration tables (<300ms). A scrub HAS NO DURATION; scroll
                 position is the clock. Also out: the frequency table, button
                 :active scale, tooltip delays, popover transform-origin,
                 keyboard-action rules, drag/gesture momentum, and the Sonner
                 component-library principles.

     EASING CONFLICT — do NOT adopt this file's literal curve values. The repo
     already defines its motion vocabulary once, on purpose:
         styles/tokens.css   --ease-out-sharp: cubic-bezier(.16,1,.3,1)
         components/motion/motion.ts   MOTION.ease = "expo.out"
     Those already satisfy this skill's PRINCIPLE (custom curve, never a weak
     built-in, never ease-in). Introducing cubic-bezier(0.23,1,0.32,1) alongside
     them gives the page two easing languages, which is exactly what motion.ts
