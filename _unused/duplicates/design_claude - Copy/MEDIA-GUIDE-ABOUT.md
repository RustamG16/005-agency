...
Specs: **1:1, 1080×1080, 6–8s, seamless loop**, constant motion speed, no cuts, no
flicker, no baked text or logos, no people's faces, no red (accent is code-only, R5).
...
Post: trim to the cleanest loop point, then
`ffmpeg -i in.mp4 -an -vf "crop=..." -c:v libx264 -crf 20 -pix_fmt yuv420p -movflags +faststart loopX.mp4`.
