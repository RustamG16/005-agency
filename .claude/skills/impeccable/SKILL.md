...
> **ADAPTER NOTE — read this first (added by the /about-v3 implementation run).**
>
> This is the canonical `impeccable` skill installed as a project skill. The
> bundled adapted copy named in `IMPLEMENTATION-PLAN.md §1.2` was not present in
> `.olympus/about-v3/media/whatwedo/implementation_aboutUs/skills/`, so the
> canonical SKILL.md was installed instead and this header was reconstructed.
>
> **`scripts/context.mjs` is NOT bundled.** Do not run it. Read these instead,
> in this order, and treat them as the Setup step's output:
> `CLAUDE.md` (hard rules — these outrank the skill), `DESIGN.md` (tokens, the
> only source of visual values), `styles/tokens.css` (executable form),
> `PRODUCT.md`, and `symbol-studio-design-audit.md` for measured geometry.
>
> **`reference/*.md` is NOT bundled.** Every `[reference/x.md]` link below is
> unresolvable. Where the skill defers to a reference file, apply the principles
> stated inline in this file plus the repo documents above, and continue — a
> missing reference is never a reason to stop.
>
> **Mode for this run: Persuade** (`/about-v3` is a marketing surface).
> **Discipline for this run: refinement, not redesign** — preserve the incumbent
> identity, copy and everything out of scope. Do not replace `DESIGN.md`.
>
> **The stop rule below is binding and this run is unattended:** build fully,
> inspect once in a batched desktop+mobile round, fix everything in one batch,
> confirm with at most one more round, then stop.
